...
const BookAppointment = (props) => {
  ...
  return (
    <section style={{ marginTop: "50px" }}>
      <div className="py-5">
        <h2 className="text-center app-color">
          Available Appointment on {props.date.toDateString()}
        </h2>
      </div>
      <div className="d-flex justify-content-center">
        <div className="booking-card row w-75">
          {bookingData.map((data) => (
            <BookingCard data={data} date={props.date}></BookingCard>
          ))}
        </div>
      </div>
    </section>
